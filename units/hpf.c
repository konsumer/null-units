...
float process(uint8_t position, float input, uint8_t channel, float sampleRate, double currentTime) {
    ...
    lastOutput[channel] = alpha * input + (1.0f - alpha) * lastOutput[channel];
    float highPassOutput = input - lastOutput[channel];

    // Ensure output is valid
    if (isnan(highPassOutput)) highPassOutput = 0.0f;
    if (isinf(highPassOutput)) highPassOutput = 0.0f;
